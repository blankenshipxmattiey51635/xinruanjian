...
class MainWindow(QMainWindow):
    ...
    def __init__(self, db: Database, settings_store: SettingsStore, settings: AppSettings) -> None:
        ...
        self.setWindowTitle("个人炒股辅助软件 - 第一阶段")
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self.watch_table.setHorizontalHeaderLabels(["代码", "名称", "市场", "当前价", "涨跌幅", "数据状态"])
    ...
    def _build_center_panel(self) -> QWidget:
        tabs = QTabWidget()
        tabs.addTab(self._placeholder_chart("分时图", "第二阶段接入真实行情后显示分时走势；当前禁止使用随机/假行情。"), "分时")
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self.news_text.setText("暂无真实数据：新闻/公告/研报标题抓取将在第四阶段接入真实数据源。")
    ...
    def show_setup_wizard(self) -> None:
        ...
        if wizard.exec() == SetupWizard.Accepted:
            ...
            self.status.showMessage("设置已保存。下一阶段接入真实行情源后将按配置刷新。", 8000)
    ...
    def reload_watchlist(self) -> None:
        ...
        for row, item in enumerate(items):
            values = [item.symbol, item.name, item.market, "--", "--", "暂无真实数据"]
    ...
    def refresh_quotes(self) -> None:
        ...
        for row, item in enumerate(items):
            ...
            if result.ok and isinstance(result.data, dict):
                self.watch_table.setItem(row, 3, QTableWidgetItem(str(result.data.get("price", "--"))))
                self.watch_table.setItem(row, 4, QTableWidgetItem(str(result.data.get("change_percent", "--"))))
                self.watch_table.setItem(row, 5, QTableWidgetItem(result.source))
            else:
                self.watch_table.setItem(row, 3, QTableWidgetItem("--"))
                self.watch_table.setItem(row, 4, QTableWidgetItem("--"))
                self.watch_table.setItem(row, 5, QTableWidgetItem(result.message or "数据源不可用"))
        self.status.showMessage(f"自选股刷新周期：{self.settings.quote_refresh_seconds} 秒；未接真实数据源时不显示伪造行情。", 5000)
